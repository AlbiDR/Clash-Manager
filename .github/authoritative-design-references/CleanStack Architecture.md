...
- **Visual Purity (Zero-Library Dependency):** No third-party icon libraries or emojis. Use custom-crafted SVG paths for absolute stylistic control and 100/100 performance. All SVG paths must use `vector-effect="non-scaling-stroke"` to maintain consistency across scales, and must be rendered exclusively via the `@shared/ui/Icon.vue` primitive to unify CSS variable access. This principle governs shipped/runtime code only; devDependency-only build tooling (e.g. Playwright, used solely for build-time skeleton geometry capture in `scripts/capture_skeletons.ts` and never bundled into the shipped app) is out of scope, consistent with the existing `shot-scraper` CI-only precedent in `generate-branding.yml`.
