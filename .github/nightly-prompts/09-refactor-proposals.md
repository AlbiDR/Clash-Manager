...
- **CLEAN Evidence Floor:** A clean run must name the priority checks actually performed (duplicate detection, size audit, layer violation scan), the candidate set inspected (`/tmp/nightly/changed-files.txt`, `/tmp/nightly/dep-violations.txt`, or the intelligence sections), and why no extraction or boundary repair was viable. Do not finalize with only "no structural debt found".
