...
- If no structural debt is found within the bounded candidate set, skip source edits and finalize `CLEAN` using the CLEAN Evidence Floor format above. This bullet grants no exemption from it: "one concise summary" is not a licence to shorten, and "no structural debt found" is the phrase the Floor explicitly forbids. Transcribe what you opened.
