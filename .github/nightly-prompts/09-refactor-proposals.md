...
- **New PR Entry Format (T1):** When appending this run's record to `pr-history.md`, write it as a full T1 block at the top of the T1 section: `### [YYYY-MM-DD] PR #N [Stage 9]: type(scope): title` / `**Domain:** [domain] | **Commit:** hash | [View PR](url)` / `**Files:** path/to/changed/file` / `**Why:** [one sentence]` / `**Change:** [one sentence]` / `**Result:** [measured or expected outcome]`.
...
- **Active Intelligence Check:** Before selecting a refactoring target, read `.github/nightly-logs/pipeline-intelligence.md` (especially Section I, III, and V) and the T1 active section of `pr-history.md`. You must check Section I to verify whether a pattern or central utility has already been established (e.g. game asset resolution or timing constants) and check Section III (Scope Coverage Map) to avoid target collision with files Stage 4 (Optimization) has modified or cleaned in the last 7 days.
