...
- **Dead-export scan (`/tmp/nightly/knip.txt`):** depcruise finds a dependency pointing the wrong way; knip finds one that should not exist. Unused exports, unused types and duplicate exports are structural debt of exactly the kind this stage owns. Two rules govern it:
  - **A knip finding is a candidate, never a verdict.** It is a static tool and it cannot see an export a framework discovers by name at runtime. `useClashDataLoader` in the feature views is the standing example: it is exported at module level so Vue Router's `DataLoaderPlugin` can find it, so knip reports it as unused on every run and deleting it breaks the route. Before removing ANY export, grep the repo for its name and confirm no plugin, router, or config resolves it dynamically. If you cannot prove it is dead, leave it and say so.
  - **A file that says it was not scanned is not a clean file.** `knip.txt` states its own reason when it did not run. Report what it actually says; never treat an unread or skipped scan as zero findings.
- **CLEAN Evidence Floor:** A clean run is reported by transcription, not by composition. Before finalizing, open `/tmp/nightly/changed-files.txt`, `/tmp/nightly/dep-violations.txt` and `/tmp/nightly/clean-calibration.txt`. Build the summary by copying literal values out of what you just opened: (1) how many candidate files `changed-files.txt` contained, the violation count in `dep-violations.txt`, and the count of each category in `knip.txt` (unused exports, unused types, duplicate exports) or the reason that file gives for not having scanned; (2) the `consecutive-clean` value from `clean-calibration.txt`; (3) the specific modules you opened, named one by one, and no surface wider than those; (4) the one candidate that came closest to being a refactor and why it was not viable, AND the one surface you hunted for defects with what you expected to break and why the test you wrote passed instead. A clean summary must contain at least one value that would have been different on a different night. If your summary would be byte-identical to the previous CLEAN row in this coverage log, you have not done (2) or (4) yet.
