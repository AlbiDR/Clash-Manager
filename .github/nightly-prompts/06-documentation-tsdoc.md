...
- **New PR Entry Format (T1):** When appending this run's record to `pr-history.md`, write it as a full T1 block at the top of the T1 section: `### [YYYY-MM-DD] PR #N [Stage 6]: type(scope): title` / `**Domain:** [domain] | **Commit:** hash | [View PR](url)` / `**Files:** path/to/changed/file` / `**Why:** [one sentence]` / `**Change:** [one sentence]` / `**Result:** [measured or expected outcome]`.
...
- **Active Intelligence Check:** Before selecting an annotation target, read `.github/nightly-logs/pipeline-intelligence.md` (specifically Section V Stage 6 context) and check the active T1 section in `pr-history.md`. Prioritize targeting files recently modified by Stage 1 (Harden) or Stage 4 (Optimize) in T1, and reconcile contracts for core services marked under the Stage 6 Focus area in Section V.
