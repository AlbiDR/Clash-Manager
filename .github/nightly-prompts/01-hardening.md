...
- **History aging:** Before scanning source, run `node .github/scripts/nightly/age-pr-history.mts age "$(cat /tmp/nightly/TODAY)"`. Do not read or edit `00-pr-history.md` manually. If aging fails, continue the security audit and mention the deferred aging in the final summary; aging must not consume the run.
...
- **CLEAN Evidence Floor:** A clean run must name the priority-list items actually checked (unauthenticated endpoints, in-memory state, Valibot boundary gaps, dead critical-path code, cross-layer violations), the changed-file count or fallback surface inspected, and why no threat was viable. Do not finalize with only "no threat found" or "audit complete".
