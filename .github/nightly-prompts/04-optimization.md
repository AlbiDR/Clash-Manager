...
1. Start with `node .github/scripts/nightly/nightly-stage.mjs start --stage 4`.
...
3. After target selection and immediately before and after required verification, run `node .github/scripts/nightly/nightly-stage.mjs budget --stage 4`. If it prints `SUBMIT`, stop source work and follow the fallback rules in `.github/nightly-prompts/00-nightly-agent-contract.md`.
4. Finalize with `node .github/scripts/nightly/nightly-stage.mjs finalize --stage 4 --status <CHANGED|CLEAN|SKIPPED|PARTIAL-RUN> --summary "<what changed>" --why "<rationale>" --result "<verification result>"`.
...
- **CLEAN Evidence Floor:** A clean run must say what it actually inspected. Name the changed-file count or the fallback surface, the priority family checked (structural rot, substrate hygiene, or duplicate logic), the command/source search used, and why no optimization was safe or useful. Do not finalize with only "no bottleneck found" or "no source changes required".
- **CLEAN Calibration Gate:** Read `/tmp/nightly/clean-calibration.txt` before finalizing. If it says `calibration-due: YES` and the normal recent-file scan finds no bottleneck, widen the scan to one older optimization surface inside the Stage 4 priority list. A calibration CLEAN summary must name the widened surface, the ordinary CLEAN-since-calibration count, and the reason no optimization was safe or useful.
...
- Run `node .github/scripts/nightly/nightly-stage.mjs budget --stage 4`, then `node .github/scripts/nightly/nightly-stage.mjs finalize --stage 4 --status <STATUS> --summary "<what changed>" --why "<rationale>" --result "<verification result>"`.
