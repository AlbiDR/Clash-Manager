...
- **New PR Entry Format (T1):** When appending this run's record to `pr-history.md`, write it as a full T1 block at the top of the T1 section: `### [YYYY-MM-DD] PR #N [Stage 4]: type(scope): title` / `**Domain:** [domain] | **Commit:** hash | [View PR](url)` / `**Files:** path/to/changed/file` / `**Why:** [one sentence]` / `**Change:** [one sentence]` / `**Result:** [measured or expected outcome]`.
...
- **Active Intelligence Check:** Before selecting a target, read `.github/nightly-logs/pipeline-intelligence.md` (specifically Section I, II, and III) and check the active T1 section in `pr-history.md`. You must avoid modifying files recently refactored or proposed for refactoring by Stage 9 this week, and you must check the Scope Coverage Map (Section III) to prevent redundant work on files already optimized or marked clean in this cycle.
