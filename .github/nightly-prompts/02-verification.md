...
4. Finalize with `node .github/scripts/nightly-stage.mjs finalize --stage 2 --status <CHANGED|CLEAN|SKIPPED|PARTIAL-RUN> --summary "<what changed>" --why "<rationale>" --result "<verification result>"`.
...
- Run `node .github/scripts/nightly-stage.mjs budget --stage 2`, then `node .github/scripts/nightly-stage.mjs finalize --stage 2 --status <STATUS> --summary "<what changed>" --why "<rationale>" --result "<verification result>"`.
