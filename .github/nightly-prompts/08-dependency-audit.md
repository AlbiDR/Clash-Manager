...
- **Branch Naming Schema:** The working branch created for your PR must follow the schema: `nightly/stage-[stage_number]-[stage_kebab_name]-[random_hash]` (e.g., `nightly/stage-8-dependency-audit-a1b2c3d4`).
- **Standard Log Format:** Every log entry written to `.github/nightly-logs/` must follow the format: `* [YYYY-MM-DD] [Stage N] Target: [file_path_or_scope] - [imperative action statement]` (e.g., `* [2026-07-13] [Stage 8] Target: package.json - Updated Valibot to patch release v0.31.1.`).
- **PR History Pruning:** At the start of your run, check the file size of `.github/nightly-logs/PR_HISTORY.md`. If it exceeds 1MB, perform in-place smart pruning: retain the 30 most recent PR blocks (`## [Date] PR #[Num]: ...` through to their `---` separators) in full detail under the "Recent Execution Details" section, and aggregate all older entries into single-line markdown list items format `* [YYYY-MM-DD] PR #[Num]: [Title] (Commit: [hash_prefix])` under the "Historical PR Index" section. This keeps history light, clean, and context-rich.
...
Audit all package.json files across the monorepo. If no changes are needed, proceed directly to Step 3 to write only the log entry (skip all package modification and test execution sub-steps in Step 3), then proceed to Step 4 to submit a no-action-required PR. Do not exit early or skip the PR, as logging the audit pass is required.
- **Priority List:**
