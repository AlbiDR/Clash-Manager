...
- **PR History Rotation:** At the start of your run, check the file size of `.github/nightly-logs/PR_HISTORY.md`. If it exceeds 1MB, archive it by renaming it to `PR_HISTORY_archive_[YYYY]_[index].md` (using the current year and the lowest available sequential integer index starting at 1, e.g., `PR_HISTORY_archive_2026_1.md` if no archive exists, or `PR_HISTORY_archive_2026_2.md` if `_1` exists), then create a new empty `.github/nightly-logs/PR_HISTORY.md` file.
