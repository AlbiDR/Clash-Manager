...
- **Read Pipeline Intelligence:** At the start of your run, read `.github/nightly-logs/PIPELINE_INTELLIGENCE.md` in full. Use it to avoid repeating tried approaches, follow proven patterns for this domain, and stay aware of open constraints and scope saturation. The PR_HISTORY.md aging pass is handled by Stage 1 and must not be performed by this stage.
...
- **New PR Entry Format (T1):** When appending this run's record to `PR_HISTORY.md`, write it as a full T1 block at the top of the T1 section: `### [YYYY-MM-DD] PR #N [Stage 8]: type(scope): title` / `**Domain:** [domain] | **Commit:** hash | [View PR](url)` / `**Files:** path/to/changed/file` / `**Why:** [one sentence]` / `**Change:** [one sentence]` / `**Result:** [measured or expected outcome]`.
