...
- **New PR Entry Format (T1):** When appending this run's record to `00-pr-history.md`, write it as a full T1 block inside the T1 section. You MUST perform this insertion exclusively by running the following python script via shell command (substituting values as needed):
  `./.github/scripts/age_pr_history.py add "$TODAY" "Stage 11" "[domain]" "PENDING" "path/to/changed/file" "Why description" "Change description" "Result description"`
...
- **CLEAN Pass Build Bypass (Time Budget Preservation):** If the scan produces a CLEAN audit pass (zero source files modified), you MUST skip the local compilation and PWA production build. Instead, read the pre-computed test baseline `/tmp/nightly/baseline-test-state.txt`. If it contains `PASS`, write `PASS (pre-compiled test baseline)` in your PR description. Only execute `pnpm build` or native compilation if functional changes were actually applied to the source files.
