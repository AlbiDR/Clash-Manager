...
> `AGENTS.md` is the sole shared lifecycle contract. This prompt contains only Stage 7 scope and execution instructions.
...
3. After target selection and immediately before and after required verification, run `node .github/scripts/nightly-stage.mjs budget --stage 7`. If it prints `SUBMIT`, stop source work and follow the fallback rules in `AGENTS.md`.
4. Finalize with `node .github/scripts/nightly-stage.mjs finalize --stage 7 --status <CHANGED|CLEAN|SKIPPED|PARTIAL-RUN> --summary "<concise result>"`.
...
- Run `node .github/scripts/nightly-stage.mjs budget --stage 7`, then `node .github/scripts/nightly-stage.mjs finalize --stage 7 --status <STATUS> --summary "<concise result>"`.
