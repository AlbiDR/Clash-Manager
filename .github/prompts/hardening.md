...
### [B] Step 2: Shadow Mode (Threat Proof)
...
* **[1] Title Schema:**
* **[a]** `fix(security): [summary]` (auth gaps, exposed endpoints)
* **[b]** `fix(reliability): [summary]` (persistence, state lifecycle)
* **[c]** `fix(integrity): [summary]` (version drift, health check)
* **[d]** `fix(validation): [summary]` (missing schema boundaries)
* **[e]** `chore(prune): [summary]` (dead or misleading code removal)
* **[f]** `chore(harden): no threat found` (exhausted priority list with no actionable runtime risk)
* **[2] Description Schema:**
* **[a]** **Prompt Source:** `Generated by: .github/prompts/hardening.md`
* **[b]** Threat Statement (from Shadow Mode — "If X, then Y, because Z") or "None — priority list exhausted" with a summary of what was inspected
* **[c]** Blast Radius (what breaks or leaks if unfixed)
