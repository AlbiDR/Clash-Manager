 ...
 * 6. Emoji Ban
 *
 * The project bans emoji outright, in the product and in everything written
 * around it. Until now that ban was stated in three places and enforced in
 * none: release-schema.json tells whoever writes the release notes "No emojis",
 * and checkDatabaseBaseline rejects non-BMP characters in one SQL file. Nothing
 * looked at the other several thousand. Three had accumulated - a console.log
 * in the entry synthesiser that printed on every dev server start, a blocked
 * message in an APK hook, and a warning sign in a core type docblock.
 *
 * [DECISION LOG] Matching is Emoji_Presentation, not "anything non-ASCII" and
 * not the whole Misc Symbols block. The narrow definition is what makes the
 * check usable: arrows, box drawing and check marks are how the scripts in this
 * repo already draw their output, and a rule that condemned those would be
 * turned off within a week. Anything carrying U+FE0F counts regardless of its
 * base character, because that selector exists precisely to ask for the emoji
 * rendering. Player names arriving from the game API are data and never reach
 * here; this walks authored files only.
 */
function checkNoEmoji() {
  log.header('6. Emoji Ban');

  const DEFAULT_EMOJI =
    /[\u{1F000}-\u{1FAFF}]|[\u231A-\u231B\u23E9-\u23EC\u23F0\u23F3\u25FD-\u25FE\u2614-\u2615]|[\u2648-\u2653\u267F\u2693\u26A1\u26AA-\u26AB\u26BD-\u26BE\u26C4-\u26C5]|[\u26CE\u26D4\u26EA\u26F2-\u26F3\u26F5\u26FA\u26FD\u2705\u270A-\u270B]|[\u2728\u274C\u274E\u2753-\u2755\u2757\u2795-\u2797\u27B0\u27BF]|[\u2B1B-\u2B1C\u2B50\u2B55]|.\uFE0F/u;

  // .claude is gitignored local tooling, so it is not part of the project this
  // rule governs and would fail here while passing in CI.
  const SKIP_DIRS = new Set([
    'node_modules', '.git', 'dist', 'build', '.turbo', 'coverage', '.vite', 'APK',
    '.claude',
  ]);
  // Generated by tooling rather than written by anyone, so its contents are not
  // an authoring decision this check can meaningfully hold someone to.
  const SKIP_FILES = new Set(['test-results.json', 'pnpm-lock.yaml']);
  const EXTS = new Set([
    '.ts', '.tsx', '.js', '.mjs', '.vue', '.json', '.md', '.yml', '.yaml',
    '.sql', '.html', '.css',
  ]);
  ...
  const walk = (dir: string): void => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      lines.forEach((line, index) => {
        const hit = line.match(DEFAULT_EMOJI);
        if (hit) {
          findings.push(
            `${path.relative(ROOT_DIR, full)}:${index + 1}  ${hit[0]}  ${line.trim().slice(0, 72)}`,
          );
        }
      });
    }
  ...
  if (findings.length > 0) {
    for (const finding of findings) log.fail(`[EMOJI] ${finding}`);
    log.fail(`${findings.length} emoji found across ${scanned} authored files.`);
    hasFailure = true;
    return;
  }

  log.pass(`No emoji in ${scanned} authored files.`);
}
