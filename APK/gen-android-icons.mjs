 ...
 *
 *   • background  -> @color/colorPrimary (#0b0e14, already in the project)
...
/** Brand background - equals @color/colorPrimary, splash and theme color. */
const BG_COLOR = "#0B0E14";

/**
 * Target res tree: the committed, apktool-decoded `android/` project (the real
 * source of truth that preserves the custom native layer). NOT
 * ~/bubblewrap-project, which is a generic decoy. Override with ANDROID_RES_DIR.
 */
const RES_DIR =
  process.env.ANDROID_RES_DIR || path.join(__dirname, "android/res");
...
/**
 * apktool pins resource IDs in res/values/public.xml; any new resource must be
 * declared there or `apktool b` fails. Declare the three new launcher mipmaps
 * (idempotent). ic_maskable is intentionally left in place - the compiled R
 * class references its id, so removing it would break the build.
 */
function ensurePublicSymbols() {
  const pub = path.join(RES_DIR, "values/public.xml");
  if (!fs.existsSync(pub)) {
    console.log("  • no public.xml (not an apktool tree) - skipped");
    return;
  }
  let s = fs.readFileSync(pub, "utf8");
  if (s.includes('name="ic_launcher_foreground"')) {
    console.log("  • public.xml symbols already present");
    return;
  }
  const ids = [...s.matchAll(/<public type="mipmap"[^>]*id="0x([0-9a-fA-F]+)"/g)]
    .map((m) => parseInt(m[1], 16));
  let next = (ids.length ? Math.max(...ids) : 0x7f0d0000) + 1;
  const hex = (n) => "0x" + n.toString(16).padStart(8, "0");
  const adds = ["ic_launcher_foreground", "ic_launcher_monochrome", "ic_launcher_round"]
    .map((n) => `    <public type="mipmap" name="${n}" id="${hex(next++)}" />`)
    .join("\n");
  const last = s.lastIndexOf('<public type="mipmap"');
  const eol = s.indexOf("\n", last);
  s = s.slice(0, eol + 1) + adds + "\n" + s.slice(eol + 1);
  fs.writeFileSync(pub, s);
  console.log("  ✓ public.xml: declared ic_launcher_foreground/monochrome/round");
}
...
async function main() {
  ...
  // 4. Declare new mipmap symbols in apktool's public.xml -------------------
