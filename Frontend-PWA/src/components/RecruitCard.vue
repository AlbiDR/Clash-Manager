...
<template>
  ...
  >
    ...
    <template #identity-name>
        ...
        <div class="trophy-meta hit-target" v-tooltip="getTooltip('trophies', recruit.t)">
    ...
    <template #score-section>
        <span class="stat-score" v-tooltip="getTooltip('score', recruit.s)">{{ Math.round(recruit.s || 0) }}</span>
    </template>
...
.btn-action { flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px; height: 44px; border-radius: 12px; font-size: 13px; font-weight: 700; text-decoration: none; border: none; cursor: pointer; background: var(--sys-color-surface-container-highest); color: var(--sys-color-on-surface); }
