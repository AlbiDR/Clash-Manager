...
<template>
    <div class="selection-bar animate-pop" :aria-busy="loading ? 'true' : 'false'">
        <template v-if="loading">
            <div class="sk-text-line-m skeleton-anim" style="width: 100px; flex-shrink: 0;"></div>
...
.action-chip { 
    background: none; border: none; font-weight: 700; cursor: pointer; padding: 6px 10px; border-radius: 8px; 
    white-space: nowrap; font-size: 13px; color: var(--sys-color-outline); transition: all 0.2s; flex-shrink: 0;
}
...
.v-divider { width: 1px; height: 16px; background: var(--sys-color-outline-variant); margin: 0 2px; flex-shrink: 0; }

.score-group { 
    display: flex; align-items: center; gap: 4px; background: var(--sys-color-surface-container-high); 
    padding: 3px; padding-left: 8px; border-radius: 99px; border: 1px solid transparent; transition: all 0.3s ease;
    flex-shrink: 0;
}
