...
- **Dependencies**:
  - `@core/api/VoyageClient`: Specialized transport for event activation and ledger fetching.
  - `@core/api/SupabaseClient`: Infrastructure-level realtime bridge.
...
2. **Activation/Scheduling**: User triggers an action -> `useVoyageForm` invokes `useVoyageStore` -> RPC (via `VoyageClient`) executed on Supabase.
...
- **Cross-Feature Injection**: To maintain Layer 3 isolation while allowing the banner to appear in the Roster view, `VoyageBanner` is explicitly imported by `RosterView.vue`. Configuration components are asynchronously loaded in `SettingsView.vue` to optimize initial bundle size.
