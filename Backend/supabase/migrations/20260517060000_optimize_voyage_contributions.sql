...
-- Delete ghost records from old testing (people not in clan with 0 manual crowns)
DELETE FROM drivers.clan_voyage_contributions 
WHERE COALESCE(manual_crowns, 0) = 0 
  AND player_tag NOT IN (SELECT player_tag FROM drivers.members);

-- Replace the refresh function to use an UPSERT and INNER JOIN drivers.members
