...
/**
 * Migration: Fix war rate percentage calculation
 * -----------------------------------------------------------------------
 * Root Cause: `avg(wa.decks_used) / 4.0 * 100.0` resulted in percentages up to 400%
 * because there are 16 available decks per war week (4 days * 4 decks), not 4.
 * Fix: Use 16.0 as the divisor for the percentage calculation to normalize the
 * war participation rate to a proper 0-100% scale. To maintain the exact same
 * baseline score weighting, the multiplier for `war_rate` in the score formulas
 * is increased from 150.0 to 600.0 (4x).
 */
