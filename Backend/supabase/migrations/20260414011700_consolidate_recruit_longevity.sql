...
-- 1. Consolidate longevity tracking into a single authoritative integer (minutes) for Legacy Parity
DROP VIEW IF EXISTS drivers.top_recruits_view;
DROP VIEW IF EXISTS features.headhunter_view;

-- 2. Create features.headhunter_view with consolidated longevity
CREATE OR REPLACE VIEW features.headhunter_view AS
 ...
 SELECT r.tag,
    ...
    r.war_wins,
    r.found_date,
    round((EXTRACT(EPOCH FROM (now() - r.found_date)) / 60))::integer AS longevity, -- Precise Minute Counter
