...
/**
 * Migration: Restore hist Column in features.scoring_view / roster_view
 * -----------------------------------------------------------------------
 * Root Cause: 20260516200000_clan_voyage_scoring_kernel.sql rebuilt
 * features.scoring_view without the hist subquery that was introduced
 * in 20260511133400_restore_unlimited_window_and_lifetime_stats.sql.
 * As a result, features.roster_view had no hist column to select, and
 * the PWA MemberCard WarHistoryChart received an empty string for every
 * member, rendering "No history" universally.
 *
 * Fix: Rebuild features.scoring_view to re-add the hist string_agg
 * subquery inside factual_logs and expose it in the final SELECT.
 * Then rebuild features.roster_view to propagate s.hist through.
 * All voyage_index / voyage_merit logic from the Clan Voyage migration
 * is preserved intact.
 */
...
-- =============================================================================
-- SCORING VIEW (Clan Voyage kernel + hist restoration)
-- =============================================================================
 ...
 WITH voyage_history AS (
  ...
  factual_logs AS (
         SELECT
            ...
            -- Restored: serialized war history string for the PWA chart
...
-- =============================================================================
-- ROSTER VIEW (exposes s.hist alongside all existing columns)
-- =============================================================================
