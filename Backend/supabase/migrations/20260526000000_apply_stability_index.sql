...
/**
 * Migration: Apply Stability Index to Performance Metrics
 * -----------------------------------------------------------------------
 * Root Cause: The `stability_index` was calculated but never applied
 * to the scoring formula. This allowed members with a single perfect
 * week (like HooooMa) to achieve a 100 PeS purely because they had
 * a 100% win rate and high fame over a tiny sample size of 1 week.
 *
 * This migration implements the fix by applying the stability index
 * (recorded_weeks / max_history_weeks) as a dampening multiplier to the
 * highly volatile time-averaged metrics (`avg_fame` and `war_rate`).
 * It applies it both in the member scoring and in the clan baseline.
 */
         ...
         SELECT
            ...
            ( SELECT max(s.baseline_raw_score)
                FROM ( SELECT round(
                                ...
                                + COALESCE(fl2.avg_fame, 0::numeric) * 15.0 * LEAST(1.0, COALESCE(fl2.recorded_weeks, 0)::numeric / bcb.max_history_weeks::numeric)
                                + m.donations::numeric * 100.0
                                + m.trophies::numeric * 0.1
                                + COALESCE(fl2.avg_war_rate, 0::numeric) * 600.0 * LEAST(1.0, COALESCE(fl2.recorded_weeks, 0)::numeric / bcb.max_history_weeks::numeric)
