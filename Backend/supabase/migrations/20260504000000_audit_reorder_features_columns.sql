...
CREATE VIEW features.headhunter_view AS
 ...
 SELECT 
    ...
    CASE
        WHEN (bc.raw_potential_score >= (12000)::numeric) THEN 'ELITE'::text
        WHEN (bc.raw_potential_score >= (10500)::numeric) THEN 'HIGH'::text
        ELSE 'MID'::text
    END AS tier,
    CASE
    ...
    COALESCE((bc.is_fresh AND (bc.max_pes > 10000)), false) AS has_heritage_blessing,
    bc.last_seen_at,
    bc.found_date,
