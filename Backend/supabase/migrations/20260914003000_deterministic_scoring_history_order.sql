...
-- Give the history orderings a tiebreaker. ORDER BY max_recorded / end_at alone
-- is not a total order, so tied rows came back in plan order: hist and v_hist
-- reordered between identical queries, and the same ordering assigns
-- recency_rank, the WEIGHT in weighted_avg, so tied weeks could swap weights
-- and move avg_fame and every score with no data change. week_id is unique per
-- player; voyage adds voyage_id then crowns; donation was already total.
