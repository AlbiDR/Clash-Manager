...
/**
 * Migration: Restore Scoring Stability Dampening and Donation Weight
 *
 * Rationale:
 * - Restore the stability index dampening factor (LEAST(1.0, recorded_weeks / max_history_weeks))
 *   for volatile metrics (avg_fame, war_rate) to prevent low-sample players from inflating the leaderboard.
 * - Restore the donation weight in the core baseline formula to 115.0 to align with design decisions.
 */
...
-- 1. Rebuild features.scoring_view with stability dampening and 115.0 donation weight
...
-- 3. Rebuild features.voyage_contributions
...
-- 4. Rebuild features.voyage_summary
