...
-- 1. Re-drop views to allow high-fidelity longevity granularity
DROP VIEW IF EXISTS drivers.top_recruits_view;
DROP VIEW IF EXISTS features.headhunter_view;

-- 2. Create features.headhunter_view with high-fidelity longevity (minutes + days)
CREATE OR REPLACE VIEW features.headhunter_view AS
 ...
 SELECT r.tag,
    ...
    r.war_wins,
    r.found_date,
    round((EXTRACT(EPOCH FROM (now() - r.found_date)) / 60))::integer AS longevity_minutes,
    EXTRACT(DAY FROM (now() - r.found_date))::integer AS longevity_days,
...
-- 3. Update drivers.top_recruits_view
